...
def compute_features(dem):
    """5-channel stack [detrend_4, detrend_12, slope, lap, locallow], identical
    to cnn_dataprep. detrend_4 (fine-scale) is the 'faint depressions' channel
    the 5ch agricultural model uses. 4-channel models (v14, river) take channels
    1:5 via the predict-time slice, so their behaviour is unchanged."""
    a = dem.astype(np.float32)
    detrend4 = a - _gaussian_blur(a, 4.0)
    detrend = a - _gaussian_blur(a, 12.0)
    ...
    locallow = (neigh - a).astype(np.float32)
    # explicit assignment (works on the old NumPy 1.9 in ArcMap 10.8,
    # which has no np.stack)
    out = np.empty((5,) + a.shape, dtype=np.float32)
    out[0] = detrend4
    out[1] = detrend
    out[2] = slope
    out[3] = lap
...
def unet_forward(feat, W):
    """feat: (4, H, W) already normalised. Returns prob map (H, W).
    4-level U-Net matching cnn_train.UNet (v2)."""
...
def has_orient_head(W):
    """v15: True when the weights include the orientation head."""
    return 'out_orient__weight' in W


def unet_forward_with_orient(feat, W):
    """v15: returns (seg_prob (H,W), orient (2,H,W) raw doubled-angle
    vector). Same decoder as unet_forward."""
    s1 = _conv_block(feat, W, 'e1')
    s2 = _conv_block(_maxpool2(s1), W, 'e2')
    s3 = _conv_block(_maxpool2(s2), W, 'e3')
    s4 = _conv_block(_maxpool2(s3), W, 'e4')
    ...
    orient = _conv2d(d1, W['out_orient__weight'],
                     W['out_orient__bias'], 0)
    return seg, orient
...
def predict_probability_map(dem, valid, weights, norm_mean, norm_std,
                            overlap=32, msg=None, tta=0, want_orient=False):
    ...
    want_orient : if True AND weights have the v15 orientation head, also
          stitch the doubled-angle orientation field -> returns
          (prob, orient (2,H,W)). TTA applies to segmentation only.
    """
    H, W = dem.shape
    nodata = -9999999.0
    ...
    do_orient = bool(want_orient) and has_orient_head(weights)
    prob = np.zeros((H, W), dtype=np.float32)
    orient = (np.zeros((2, H, W), dtype=np.float32) if do_orient else None)
    ...
    for r0 in range(0, max(1, H - 1), step):
        for c0 in range(0, max(1, W - 1), step):
            ...
            if feat.shape[0] != nm.shape[0]:
                # 4-channel model (v14 / river): drop detrend_4 -> channels 1:5
                feat = feat[feat.shape[0] - nm.shape[0]:]
            feat = (feat - nm) / ns
            feat = feat.astype(np.float32)
            omap = None
            if do_orient:
                pmap, omap = unet_forward_with_orient(feat, weights)
            ...
            if do_orient:
                orient[0, r0:r1, c0:c1] += omap[0][:th, :tw] * wcore
                orient[1, r0:r1, c0:c1] += omap[1][:th, :tw] * wcore
